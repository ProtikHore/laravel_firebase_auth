...
<div class="container-fluid" style="margin-top: 75px;">
    <div class="row">
        <div class="col-sm-3" ng-controller="MenuController" ng-init="getFirstLevelMenus('{{ url('get/first/level/menus') }}')">

            <table class="table table-bordered tree-basic" id="left_navigation">
                <tbody>
                <?php
                $user = \App\User::where('id', session('id'))->first();
                if($user->type == 'user') {
                    $menuIds = explode(',', $user->menu_ids);
                } else {
                    $menus = \App\Menu::get(['id']);
                ...
                foreach ($firstLevelMenus as $firstLevelMenu) {
                    if (in_array($firstLevelMenu->id, $menuIds)) {
                        ...
                        if ($firstLevelMenu->id == $activeMenuId) {
                            if ($firstLevelMenu->method == "") {
                                ...
                                echo "<tr class='treegrid-" . $i . " expanded' style='background-color: #A60A67;'><td><a href='" . url($url . '/' . $firstLevelMenu->id) . "'>" . $firstLevelMenu->menu . "</a></td></tr>";
                            }
                        } else {
                            if ($firstLevelMenu->method == "") {
                        ...
                        $secondLevelMenus = \App\Menu::where('root_id', $firstLevelMenu->id)->get();
                        if ( ! empty($secondLevelMenus)) {
                            ...
                            foreach ($secondLevelMenus as $secondLevelMenu) {
                                if (in_array($firstLevelMenu->id, $menuIds)) {
                                    ...
                                    if (isset($activeSubMenuId)) {
                                        if ($secondLevelMenu->id == $activeSubMenuId) {
                                            if ($secondLevelMenu->method == "") {
                                                ...
                                                echo "<tr class='treegrid-" . $j . " treegrid-parent-" . $i . " expanded' style='background-color: #A60A67;'><td><a href='" . url($url . '/' . $secondLevelMenu->id) . "'>" . $secondLevelMenu->menu . "</a></td></tr>";
                                            }
                                        } else {
                                            if ($secondLevelMenu->method == "") {
                                    ...
                                    $thirdLevelMenus = \App\Menu::where('root_id', $secondLevelMenu->id)->get();
                                            ...
                                            if (in_array($firstLevelMenu->id, $menuIds)) {
                                                ...
                                                if (isset($activeSubSubMenuId)) {
                                                    if ($thirdLevelMenu->id == $activeSubSubMenuId) {
                                                        echo "<tr class='treegrid-" . $k . " treegrid-parent-" . $j . "' style='background-color: #A60A67;'><td><a href='" . url($url . '/' . $thirdLevelMenu->id) . "'>" . $thirdLevelMenu->menu . "</a></td></tr>";
                                                    } else {
